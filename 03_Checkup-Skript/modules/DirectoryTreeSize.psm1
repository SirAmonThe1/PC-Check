...
if ($folderName -eq 'all') {
    ...
    $allFolders = Get-ChildItem $BasePath -Directory -Recurse -Force -ErrorAction SilentlyContinue | Where-Object {($_.FullName -notlike "*C:\Windows*")}
    [console]::beep(2000,250)
    [console]::beep(2000,250)
}
else {

    $allFolders = Get-ChildItem $basePath -Directory -Recurse -Force -ErrorAction SilentlyContinue | Where-Object {($_.BaseName -like $FolderName) -and ($_.FullName -notlike "*C:\Windows*")}
    [console]::beep(2000,250)
    [console]::beep(2000,250)
}  
...
Write-host "Zu verarbeitende Ordner:"
write-host $allFolders.Count
$var_Verzeichniscount = $allFolders.Count
...
ForEach ($folder in $allFolders) {
    ...
    $i++
    Write-Progress -Activity "Ermitteln der Ordnergroeßen" -PercentComplete (($i*100)/$allFolders.count) -Status "Aktuell bei Verzeichnis $i von $var_Verzeichniscount"
